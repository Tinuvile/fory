...
impl<T: Serializer + ForyDefault + Send + Sync + 'static> Serializer for Arc<T> {
    ...
    {
        // For Arc<T>, we cannot modify the inner value directly due to reference counting.
        // The most efficient approach is to deserialize a new Arc and replace the entire output.
