    ...
    if !skip_ref_flag {
        ...
        if ref_flag == RefFlag::Null as i8 {
            *output = T::fory_default();
            ...
            // This is a reference to a previously deserialized object
            // For now, just assign default - this should be handled by specific types
            *output = T::fory_default();
            Ok(())
        } else {
            unimplemented!("Unknown ref flag: {}", ref_flag)
...
pub trait Serializer: 'static {
    ...
    /// Read data directly into the provided output reference.
    /// This avoids creating a new instance and copying data, improving performance
    /// especially for large structures.
    fn fory_read_data_into(context: &mut ReadContext, is_field: bool, output: &mut Self) -> Result<(), Error>
    where
        Self: Sized + ForyDefault,
    {
        // Default implementation falls back to regular read and assignment
        *output = Self::fory_read_data(context, is_field)?;
        Ok(())
    }
